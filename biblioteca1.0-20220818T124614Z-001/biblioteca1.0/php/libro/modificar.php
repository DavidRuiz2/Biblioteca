<?php 
    ...
    if(isset($_POST['codigo'])) {
        ...
        if($reg = mysqli_fetch_array($registros)){
            ...
            libAutor='$autor', libEditorial='$editorial', libArea='$arNom'
            WHERE libCodigo ='$codigo'")
            or die("Problemas en el select ".mysqli_error($con));
            echo "<script>
            location.href='/punto6/views/libro.php?accion=consultar';
            alert('El registro fue modificado exitosamente');
            </script>";
        }else{
            echo "<script>
            alert('El registro no pudo ser modificado');
            location.href='/punto6/views/libro.php?accion=modificar&libNom=$codigo';
            </script>";
        }
    }
?>